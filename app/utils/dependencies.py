...
Эти функции позволяют эндпоинтам принимать данные в трёх форматах:
    ...
    Content-Type: application/json
        {
            "wallets": ["Tabc...", "Tdef..."],
            "start_timestamp": "2026-01-15 14:30",
            "end_timestamp": "2026-03-01 23:59",
            "timezone": 3
        }

    Content-Type: application/x-www-form-urlencoded
        wallets=Tabc...&wallets=Tdef...&start_timestamp=2026-01-15+14%3A30&timezone=3
...
Временные метки принимаются в форматах:
    2026-01-15 14:30             — человекочитаемый (YYYY-MM-DD HH:MM)
    2026-01-15T00:00:00Z         — ISO 8601 UTC
    2026-01-15                   — только дата (время 00:00)

Часовой пояс:
    timezone=3    → UTC+3 (Москва)
    timezone=-5   → UTC-5
    timezone=0    → UTC (по умолчанию, если не указан)
"""
...
async def _parse_body(request: Request) -> dict:
    ...
    if _is_form(request):
        ...
        # timezone из form приходит как строка — конвертируем в int
        if "timezone" in data and isinstance(data["timezone"], str):
            try:
                data["timezone"] = int(data["timezone"])
            except ValueError:
                pass  # Pydantic выдаст внятную ошибку при валидации
        return data
    else:
        try:
